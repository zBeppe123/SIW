package it.uniroma3.siw.tennis.spring.authentication;
...
import static it.uniroma3.siw.tennis.spring.model.Credentials.ADMIN_ROLE;
//import static it.uniroma3.siw.tennis.spring.model.Credentials.DEFAULT_ROLE;
...
//				.antMatchers(HttpMethod.POST, "/login", "/register", "registrazione/registraTennista").permitAll()
...
//				.logoutSuccessUrl("/index").invalidateHttpSession(true).clearAuthentication(true).permitAll();
	...
	public void configure(AuthenticationManagerBuilder auth) throws Exception {
		auth.jdbcAuthentication()
				...
				// retrieve email and role
				.authoritiesByUsernameQuery("SELECT username, role FROM credentials WHERE username=?")
				// retrieve email, password and a boolean flag specifying whether the user is
